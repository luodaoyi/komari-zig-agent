...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    --ghproxy|--install-ghproxy) github_proxy="$2"; shift 2 ;;
...
find_systemd_binary() {
  ...
  line="$(systemctl cat "${service_name}.service" 2>/dev/null | sed -n 's/^[[:space:]]*ExecStart=[[:space:]]*//p' | sed '/^$/d' | tail -n 1)"
  [ -n "$line" ] || return 1
...
if ! start_service; then
  die "service failed to start after binary update"
fi
sleep 2
if ! service_healthy; then
  die "service is not healthy after binary update"
fi

log "binary update completed"

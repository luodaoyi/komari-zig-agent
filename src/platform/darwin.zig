...
pub fn basicInfo(allocator: std.mem.Allocator) !common.BasicInfo {
    ...
    fillLocalIp(allocator, &info) catch |err| {
        debug.log("darwin local IP collection failed: {s}", .{@errorName(err)});
    };
    return info;
}
...
/// Darwin/BSD netstat appends '*' when an interface is down.
...
/// Pure ifconfig parser for unit tests on non-Darwin CI hosts.

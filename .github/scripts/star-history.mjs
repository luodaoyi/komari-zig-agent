...
function mergePoint(points, point) {
  return [...new Map([...points, point].map((item) => [item.date, item])).values()]
...
function renderSvg(history) {
  ...
  const coordinates = history.points.map((point, index) => ({ x: x(times[index]), y: y(point.stars) }));
  let line = `M ${coordinates[0].x.toFixed(1)} ${coordinates[0].y.toFixed(1)}`;
  for (const point of coordinates.slice(1)) line += ` H ${point.x.toFixed(1)} V ${point.y.toFixed(1)}`;
...
<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" role="img" aria-labelledby="title description">
  ...
  <style>
    .background { fill: #ffffff; } .grid { fill: none; stroke: #d8dee4; stroke-width: 1; }
    ...
    .heading { font-size: 20px; font-weight: 600; } .value { font-size: 24px; font-weight: 600; }
    .area { fill: #0969da; opacity: .12; } .line { fill: none; stroke: #0969da; stroke-linecap: round; stroke-linejoin: round; stroke-width: 3; }
    .point { fill: #0969da; stroke: #ffffff; stroke-width: 2; }
    @media (prefers-color-scheme: dark) {
      .background { fill: #0d1117; } .grid { stroke: #30363d; } .label, .subtitle { fill: #8b949e; }
      .heading, .value { fill: #f0f6fc; } .area { fill: #58a6ff; } .line { stroke: #58a6ff; } .point { fill: #58a6ff; stroke: #0d1117; }
    }
  </style>
  ...
  ${grid}${dates}<path class="area" d="${area}"/><path class="line" d="${line}"/>
...
async function github(pathname, token, accept = "application/vnd.github+json") {
  const headers = { Accept: accept, "User-Agent": "komari-zig-agent-star-history", "X-GitHub-Api-Version": "2022-11-28" };
...
async function readHistory(file, repository) {
  try {
    ...
    if (history.repository !== repository || !Array.isArray(history.points)) throw new Error(`${file} does not contain history for ${repository}`);
    return history;
  } catch (error) {
...
async function main() {
  ...
  if (!new Set(["snapshot", "bootstrap"]).has(command)) throw new Error("usage: star-history.mjs [snapshot|bootstrap] [output-directory]");
  const repository = process.env.GITHUB_REPOSITORY;
  if (!/^[\w.-]+\/[\w.-]+$/.test(repository ?? "")) throw new Error("GITHUB_REPOSITORY must be in owner/repository form");
  ...
  if (!points.length) throw new Error("star history has no points");
